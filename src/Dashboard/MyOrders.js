...
const MyOrders = () => {
  ...
  useEffect(() => {
    if (user) {
      fetch(`https://carhub-server.onrender.com/orders?email=${user.email}`, {
        method: "GET",
        headers: {
  ...
  const handleDelete = (id) => {
    console.log(id);
    fetch(`https://carhub-server.onrender.com/orders/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
          ...
          {myOrders.map((orders, index) => (
            <Tr>
              ...
              <Td>
                ...
                {orders.totalPrice && orders.paid && !orders.shipped && (
                  <div>
                    <span className="btn btn-xs btn-accent mr-1">Pending</span>
                    <p className="text-xs">
                      Transaction ID : {orders.transactionId}
                    </p>
                  </div>
                )}
