...
const Purchase = () => {
  ...
  const { id } = useParams();
  useEffect(() => {
    fetch(`https://carhub-server.onrender.com/parts/${id}`)
      .then((res) => res.json())
  ...
  let orderError = (
    <p className="text-xs text-red-500 ">Please enter element within range</p>
  );
  ...
  const handleBooking = (event) => {
    ...
    fetch("https://carhub-server.onrender.com/orders", {
      method: "POST",
      headers: {
  ...
  const handleChange = (event) => {
    ...
    if (
      value + 1 <= parseInt(minOrder) ||
      value - 1 >= parseInt(available) ||
      isNaN(value)
    ) {
      ...
      <div className="mt-20 lg:flex lg:justify-evenly justify-center items-center  ">
        <div class="card  bg-base-100 shadow-xl ">
          <div class="card-body justify-center border items-center">
            ...
            <p>
              Total Price:$
              {parseInt(!order ? minOrder : order) * parseInt(price)}
            </p>
            ...
            <div className="card-body">
              ...
              <form onSubmit={handleBooking}>
                ...
                <input
                  ...
                  type="submit"
                  className="btn w-full max-w-xs mt-6 btn-outline btn-secondary"
                  value="Confirm Order"
                />
              </form>
            </div>
